---
title: "milo_analysis_no_integration"
...
seurat_obj <- RunUMAP(seurat_obj, reduction = "pca", dims = 1:100)

```

```{r}
DimPlot(seurat_obj, group.by = "sample", split.by = "geno")
```

```{r}
seurat_obj$batch <- as.factor(substr(seurat_obj$orig.ident,3,3))
DimPlot(seurat_obj, group.by = "geno", split.by = "batch")

```

```{r eval = FALSE}
saveRDS(seurat_obj,"../data_output/surat_objects/run_8nov21/mito_malat_filt_no_integration_four_samples.rds")
```

Build Milo object

```{r}
milo_obj <- Milo(as.SingleCellExperiment(seurat_obj))
```

## Step 2: Build kNN graph from integrated PCA space

```{r}
milo_obj <- buildGraph(milo_obj, k = 30, d = 100, reduced.dim = "PCA")
...
```{r}
milo_obj <- makeNhoods(milo_obj, prop = 0.1, k = 30, d = 100, refined = TRUE, reduced_dims = "PCA")
...
```{r}
milo_obj <- calcNhoodDistance(milo_obj, d=100, reduced.dim = "PCA")
