...
```{r}
pdf("../data_output/20231121_fig2o_clusters_all_samples.pdf",width = 5,height = 5)
...
fzd8 <- clusterProfiler::bitr("Fzd8",fromType = "SYMBOL",toType = "ENSEMBL",OrgDb = org.Mm.eg.db::org.Mm.eg.db)$ENSEMBL
...
```
```{r}
pdf("../data_output/20231121_figS4b_fzd8_violin_plots.pdf")
VlnPlot(object = seurat_obj[,seurat_obj$seurat_clusters %in% c("0","1","10","11","9")],pt.size = FALSE, split.by = "geno",features = fzd8) +
...
```{r}
seurat_obj <- readRDS(file = "../data_output/surat_objects/tbr2_both_litters_seurat_obj.Rds")
...
litter4_metadata <- litter4_metadata[litter4_metadata$orig.ident %in% c("WT2","XP2"),]

plot_cluster_jaccard_matrix(get_cluster_jaccard_df(litter4_metadata, litter2_metadata)) + xlab("All samples") + ylab("Litter 2")
...
saveRDS(seurat_obj_conserved_markers_list_df, "../data_output/surat_objects/tbr2_both_litters_conserved_markers_list_df.rds")
...
```{r}
assignments <- readRDS("../data_output/osca_objects/cellcycle_assignments_tbr2_all_litters.rds")
...
```{r}
pdf("../data_output/20231019_ip_both_litters_split_geno_phases.pdf")
...
```
```{r}
write.csv(de_results_df,file = "../data_output/surat_objects/tbr2_clusters_deg_all.csv")
```

```{r}
write.csv(de_results_df_filt,file = "../data_output/surat_objects/tbr2_clusters_deg_fdr05_contamination_05.csv")
```
