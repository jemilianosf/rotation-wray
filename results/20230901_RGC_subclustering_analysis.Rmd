---
title: "20230901_RGC_subclustering"
...
The aim of this analysis is to sub-cluster radial glial cells to get a higher resolution clustering of this cell population.
...
Keep cells from clusters annotated as RGC intermediate progenitor neurons.

```{r}
seurat_obj$is_rgc <- seurat_obj$seurat_clusters %in% c("1","12","19","10","14","4","0")

DimPlot(seurat_obj,group.by = "is_rgc") + theme(aspect.ratio = 1)
DimPlot(seurat_obj) + theme(aspect.ratio = 1)

```


```{r eval = FALSE}
seurat_obj <- seurat_obj[,seurat_obj$seurat_clusters %in% c("1","12","19","10","14","4","0")]
...
```
```{r eval = FALSE}
saveRDS(seurat_obj,file = "../data_output/surat_objects/rgc_litter2_seurat_obj.Rds")
```

```{r}
seurat_obj <- readRDS(file = "../data_output/surat_objects/rgc_litter2_seurat_obj.Rds")

```

# Visualizing UMAP
```{r}
pdf("../data_output/20230905_rgc_litter2_umap.pdf")
...
saveRDS(seurat_obj_conserved_markers_list_df, "../data_output/surat_objects/rgc_litter2_conserved_markers_list_df.rds")
...
write.csv(top30_markers, "../data_output/surat_objects/rgc_litter2_top30lfc_markers.csv")
...
pdf("../data_output/20230905_rgc_litter2_umap_fzd8.pdf")
...
seurat_obj$malat_percent <- PercentageFeatureSet(seurat_obj,features = malat1,assay = "RNA")
...
assignments <- cyclone(sce_obj, mm.pairs, gene.names=rownames(sce_obj),assay.type = "logcounts")
saveRDS(assignments,"../data_output/osca_objects/cellcycle_assignments_litter2_rgc.rds")
...
```{r}
assignments<- readRDS("../data_output/osca_objects/cellcycle_assignments_litter2_rgc.rds")
...
```{r}
pdf("../data_output/20230905_rgc_litter2_umap_cell_cycle.pdf")

FeaturePlot(seurat_obj,features = "G1") + theme(aspect.ratio = 1)
FeaturePlot(seurat_obj,features = "G2M") + theme(aspect.ratio = 1)
FeaturePlot(seurat_obj,features = "S") + theme(aspect.ratio = 1)
