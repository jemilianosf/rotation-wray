---
title: "radial_glia_both_litters_analysis"
...
The aim of this analysis is to sub-cluster fzd8+ radial glial cells  to get a higher resolution clustering of this cell population. Using both samples to get DE genes.
...
```
```{r eval = FALSE}
saveRDS(seurat_obj,file = "../data_output/surat_objects/fzd8_both_litters_seurat_obj.Rds")
```


```{r}
seurat_obj <- readRDS(file = "../data_output/surat_objects/fzd8_both_litters_seurat_obj.Rds")
...
saveRDS(seurat_obj_conserved_markers_list_df, "../data_output/surat_objects/fzd8_both_litters_conserved_markers_list_df.rds")
...
write.csv(top30_markers, "../data_output/surat_objects/fzd8_all_samples_top30lfc_markers.csv")
...
saveRDS(assignments,"../data_output/osca_objects/cellcycle_assignments_fzd8_all_litters.rds")
```
...
```{r}
assignments <- readRDS("../data_output/osca_objects/cellcycle_assignments_fzd8_all_litters.rds")
...
```{r}
DimPlot(seurat_obj,label = T) + theme(aspect.ratio = 1)
...
```
```{r}
write.csv(de_results_df,file = "../data_output/surat_objects/fzd8_clusters_deg_all.csv")
```
