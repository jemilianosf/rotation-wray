---
title: "litter2_independent_cluster"
...
# Cluster WT and HARE5 independently
...
write.csv(top30_markers, "../data_output/surat_objects/wt2_top30lfc_markers.csv")
```

```{r eval = FALSE}
saveRDS(seurat_obj,file = "../data_output/surat_objects/wt2_seurat_obj.Rds")
```
```{r}
...
Write full list of marker genes
```{r}
markers <- FindAllMarkers(seurat_obj,min.pct = 0.25)
library(clusterProfiler)
library(org.Mm.eg.db)

symbol_map <- bitr(markers$gene,fromType = "ENSEMBL",toType = "SYMBOL",OrgDb = org.Mm.eg.db)

symbol_map <- symbol_map[!duplicated(symbol_map$ENSEMBL),]

markers <- left_join(markers,symbol_map, by =c("gene" = "ENSEMBL"))

write.csv(markers, "../data_output/surat_objects/wt2_all_markers.csv")
...
saveRDS(assignments,"../data_output/osca_objects/cellcycle_assignments_litter2_wt.rds")
```
```{r}
...
write.csv(top30_markers, "../data_output/surat_objects/hare5_top30lfc_markers.csv")
```

```{r }
saveRDS(seurat_obj,file = "../data_output/surat_objects/hare5_seurat_obj.Rds")
```
```{r}
...
write.csv(markers, "../data_output/surat_objects/hare5_all_markers.csv")
...
# Using Ensembl IDs to match up with the annotation in 'mm.pairs'.
assignments <- cyclone(as.SingleCellExperiment(seurat_obj), mm.pairs, gene.names=rownames(seurat_obj))
saveRDS(assignments,"../data_output/osca_objects/cellcycle_assignments_litter2_hare5.rds")
```
```{r}
plot(assignments$score$G1, assignments$score$G2M,
    xlab="G1 score", ylab="G2/M score", pch=16)
...
seurat_obj <- seurat_obj[,seurat_obj$sample %in% c("XP2","WT2")]
...
```{r}
DimPlot(seurat_obj,group.by = "geno",label = TRUE) + theme(aspect.ratio = 1)
```
...
write.csv(top30_markers, "../data_output/surat_objects/litter2_no_integration_top30lfc_markers.csv")
```

```{r eval = FALSE}
saveRDS(seurat_obj,file = "../data_output/surat_objects/litter2_no_integration_seurat_obj.Rds")
```
...
# Using Ensembl IDs to match up with the annotation in 'mm.pairs'.
assignments <- cyclone(as.SingleCellExperiment(seurat_obj), mm.pairs, gene.names=rownames(seurat_obj))
saveRDS(assignments,"../data_output/osca_objects/cellcycle_assignments_litter2_no_integration.rds")
```
```{r}
plot(assignments$score$G1, assignments$score$G2M,
    xlab="G1 score", ylab="G2/M score", pch=16)
